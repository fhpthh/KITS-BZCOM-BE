...
public class RequestFacadeImpl implements RequestFacade {
    ...
    public RequestResponse createRequest(CreateRequestDto dto, UserPrincipal currentUser) {
        ...
        // 3. Automatically generate alert for all ADMINs if priority is HIGH
        if (request.getPriority() == RequestPriority.HIGH) {
            log.info("(createRequest) HIGH priority request, notifying all admins");
            alertService.notifyAdmins(
                    request.getId(),
                    AlertType.HIGH_PRIORITY_REGISTERED,
                    "High priority request registered: " + request.getTitle()
            );
        }
